  ...
  renderCell: (params) => {
    ...
    const seaColumns: GridColDef[] = [
      ...
      
      {
        ...
        headerAlign: 'center',
        align: 'center',
        valueGetter: (params) => (params.row.remitted_amount !== 0 ? 'Remitted' : 'Unremitted'),
        renderCell: (cellValues) => {
              console.log(cellValues)
          return(
          <>
        {cellValues.row.remitted_amount !== 0 ? (<Chip  label={"Remitted"} color ="success" size = "small" variant = "outlined"/>) : (<Chip label={"Unremitted"} color ="warning" size = "small" variant = "outlined"/>)}
      ...
      { 
        field: 'remitted_amount', 
        headerName: 'REMITTED AMOUNT', 
         width: 180,
        headerClassName: 'super-app-theme--header',
        editable: false,
       headerAlign: 'center',
...
<div className=" bg-white border border-gray-200 rounded-lg shadow-lg p-4">  
  <div className="flex items-center">
    ...
    <div className="flex-shrink-0 ml-3">
      ...
      <h3 className="text-base font-normal text-gray-500">{"TOTAL REVENUE ATM BAGGAGE"}</h3>
    </div>
  </div>
</div>
